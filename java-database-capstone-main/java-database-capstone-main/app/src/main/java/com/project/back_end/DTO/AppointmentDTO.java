...
// - This is a simplified field, capturing only the ID of the doctor (not the
// full Doctor object).
...
// - This is a simplified field, capturing only the ID of the patient (not the
// full Patient object).
...
// - The time when the appointment is supposed to happen, stored as a
...
// - Status can indicate if the appointment is "Scheduled:0", "Completed:1", or
// other statuses (e.g., "Canceled") as needed.

// 11. 'appointmentDate' field (Custom Getter):
// - Type: private LocalDate
// - Description:
// - A derived field representing only the date part of the appointment (without
// the time).
...
// - Description:
// - A derived field representing only the time part of the appointment (without
// the date).
// - Extracted from the 'appointmentTime' field.
...
// 15. Getters:
// - Standard getter methods are provided for all fields: id, doctorId,
// doctorName, patientId, patientName, patientEmail, patientPhone,
// patientAddress, appointmentTime, status, appointmentDate,
